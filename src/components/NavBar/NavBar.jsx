...
export default function NavBar ({setUser}) {
    ...
    return (
        <>
            <div className="fixed top-0 w-screen h-16 px-6 bg-midnight-blue justify-between items-center gap-8 inline-flex z-10">
                <button id='hamburger' onClick={toggleMenu} data-collapse-toggle="navbar-default" type="button" className="inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-gray-500 rounded-lg md:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600" aria-controls="navbar-default" aria-expanded="false">
                ...
                <div className="w-40 h-8 relative block md:none">
                    <Link to="/" onClick={() => isMobile && menuOpen && setMenuOpen(!menuOpen)}>
            ...
            <div className={menuOpen === "hidden" ? "hidden" : menuOpen ? "flex z-0 bg-gradient-to-b from-midnight-blue to-rgb(255,196,126) top-16 left-0 fixed h-screen w-full flex-col p-10 animate__animated animate__slideInDown animate__fadeInDown" : "flex bg-gradient-to-b from-midnight-blue to-rgb(255,196,126) top-16 left-0 fixed h-screen w-full flex-col p-10 animate__animated animate__fadeOutUp"}>
                <hr className='bg-white mt-[-40px]' />
                <Link to="/Challenge1" onClick={() => setMenuOpen(!menuOpen)} className={location.pathname == "/Challenge1" ? navTextActive : navText}>Challenge 1</Link>
                <Link to="/Challenge2" onClick={() => setMenuOpen(!menuOpen)} className={location.pathname == "/Challenge2" ? navTextActive : navText}>Challenge 2</Link>
                <Link to="/Challenge2Bonus" onClick={() => setMenuOpen(!menuOpen)} className={location.pathname == "/Challenge2Bonus" ? navTextActive : navText}>Challenge 2 (BONUS)</Link>
                <Link to="/Challenge3" onClick={() => setMenuOpen(!menuOpen)} className={location.pathname == "/Challenge3" ? navTextActive : navText}>Challenge 3</Link>
            </div>
        </>
    )
}